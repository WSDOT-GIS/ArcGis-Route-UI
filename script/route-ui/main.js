...
define([
  ...
  var usedIds = []
  var supportsCustomEventConstructor = true
  var geocodeResultDataAttributeName = 'data-feature'
  ...
  function createId (base) {
    ...
    while (arrayContains(usedIds, output) || document.getElementById(output)) {
  ...
  function createSelect (defaultValue) {
    ...
    var avoidRe = /^Avoid/i
    var preferRe = /^Prefer/
    var re = new RegExp(defaultValue, 'i')
    var select = document.createElement('select')
    var avoidGroup = document.createElement('optgroup')
    avoidGroup.label = 'Avoid'
    select.appendChild(avoidGroup)
    var preferGroup = document.createElement('optgroup')
    preferGroup.label = 'Prefer'
    select.appendChild(preferGroup)
    var option
    var name
    for (name in restrictionParameters) {
  ...
  var ArcGisRouteUI = declare([Evented], {
    ...
    constructor: function (async) {
      ...
      var form = createFormFromObjectProperties(properties)
